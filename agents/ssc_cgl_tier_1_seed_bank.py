...
DIAGRAM PRINCIPLES — keep it simple:
  • Draw the shape with correct rough proportions. Label only what is GIVEN.
  • Mark unknowns with "?" — never their value.
  • No clutter: only draw what is needed to understand the problem.
  • For Venn diagrams: overlapping circles with set names only — no numbers in regions.
  • For geometry: vertices labelled A/B/C, given lengths on sides, right-angle box if needed.
...
STEP 2 — VERIFY THE ANSWER independently. Do the actual calculation yourself:
  QUANTITATIVE: Solve from scratch step by step. Does your final answer exactly match correct_answer?
  LOGICAL: Apply pure logic independently. Does your conclusion match correct_answer?
  CONCEPTUAL: Verify the factual accuracy of correct_answer.

STEP 3 — ANTI-CHEATING: Does the question text itself directly reveal the answer?

RESPONSE:
  Everything correct → reply with ONLY the single word: PASS
  Any issue → short numbered list. Always show your computed answer on math mismatches.
"""
...
def run_seeder():
    ...
    print(f"   Critics: Math=Sonnet (reliable geometry) | Diagram=Sonnet+Vision (visual only)")
    ...
    for subject, topic, sub_topic in TARGET_NODES:
        ...
        for difficulty in range(1, 6):
            for iteration in range(1, 3):
                ...
                total_attempts   = 0
                banked           = False
                # FIX 5: Track concepts tried across rounds so we don't repeat them
                ...
                while not banked:
                    ...
                    # Build extra hint — grows richer with each failed round
                    extra_hint = ""
                    if round_num > 1:
                        concepts_str = (
                        ...
                        extra_hint = (
                            ...
                            f"- The following concepts/theorems were already tried and failed:\n"
                            f"{concepts_str}\n"
                            f"- You MUST choose a completely different concept, theorem, or "
                            f"problem type for this subtopic and difficulty.\n"
                    ...
                    # FIX 5: Extract sub_topic from last generated question to track tried concepts
                    ...
                    if succeeded:
                        # FIX 3: Rename attempt-specific SVG to clean final name for banking
